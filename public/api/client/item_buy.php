...
if (!$live["misc"]["able_item"] || blocking_user($live["user_id"], $_SERVER["REMOTE_ADDR"], $my["acct"])) {
    api_json(["error" => "エラー: アイテムは現在使用できません。"]);
}
...
if (!check_point_true($my["point_count"], $point)) {
    api_json(["error" => "エラー: 残高が足りません。"]);
}

if ($_POST["confirm"] != 1) {
    api_json(["confirm" => true, "point" => $point]);
} //サーバー側で消費ポイントを再計算して確認させる
