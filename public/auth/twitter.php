...
if (empty($_GET["oauth_verifier"])) {
    ...
    if (empty($res["oauth_token"])) {
        showError('トークンを受信できませんでした。(1)もう一度トップページからお試しください。', 400);
    }
    ...
    if (empty($access_token["oauth_token"]) || empty($access_token["oauth_token_secret"])) {
        showError('トークンを受信できませんでした。(2)もう一度トップページからお試しください。', 400);
    }
    ...
    if (empty($content["id_str"])) {
        showError('アカウント情報が取得できませんでした。', 500);
    }
